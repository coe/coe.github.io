var ProgressView = function(dictionary) {
    ...
    (function() {
        ...
        _activityIndicator = null != Ti.Android ? Ti.UI.Android.createProgressIndicator() : Ti.UI.createActivityIndicator({
    ...
    this.show = function(dictionary) {
        if (isAndroid) {
            dictionary && (dictionary.text ? _activityIndicator.message = dictionary.text : dictionary.textId && (_activityIndicator.messageid = dictionary.textId));
            ...
            if (dictionary) {
                ...
                if (true == dictionary.success || true == dictionary.error) {
                    _hasImage = 1;
                    _imageViewStatus.image = "/modules/" + (true == dictionary.success ? "success" : "error") + ".png";
                    _viewActivityIndicator.remove(_activityIndicator);
                    _viewActivityIndicator.add(_imageViewStatus);
                }
            }
            _viewFullBackgroundActivityIndicator.opacity = 0;
    ...
    this.hide = function() {
        if (isAndroid) _activityIndicator.hide(); else {
            _viewFullBackgroundActivityIndicator.opacity = 1;
            _viewFullBackgroundActivityIndicator.animate({
                opacity: 0,
                duration: 300
            });
    ...
    this.change = function(dictionary) {
        if (!isAndroid && dictionary) {
            if (dictionary.text || dictionary.textId) if (1 == _hasText) setTextLabel(dictionary); else {
            ...
            if (true == dictionary.success || true == dictionary.error) if (1 == _hasImage) _imageViewStatus.image = "/modules/" + (true == dictionary.success ? "success" : "error") + ".png"; else {
                _hasImage = 1;
                _imageViewStatus.image = "/modules/" + (true == dictionary.success ? "success" : "error") + ".png";
                _viewActivityIndicator.remove(_activityIndicator);
                _viewActivityIndicator.add(_imageViewStatus);
    ...
    var setTextLabel = function(dictionary) {
        ...
        if (_width > 100 && 310 > _width) if (1 == _hasText) {
        ...
        } else _viewBackgroundBackActivityIndicator.width = _viewBackgroundActivityIndicator.width = _width;
